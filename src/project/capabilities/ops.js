...
• 部署：小家跑在一台 VPS 上（systemd 服务 my-ai-chat，工作目录 /opt/xiaojia），前端 dist + 接口 functions/api 都在上面；server.js 是零依赖运行时，wake/ 是唤醒引擎（运行层，不进 git）。代码仓库在 GitHub 1018-zz/my-ai-chat，改代码提交后要同步到 VPS 才生效。
...
• 外部能力口令（都不要写进对话，自己记住就好）：
  ...
  - 健康数据：健康小记从 /api/health 读，链路是 手环→Gadgetbridge→HealthBridge→后端
