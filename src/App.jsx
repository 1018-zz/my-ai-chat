...
const ChatDetailPage = ({ chatInfo, onBack, avatarSelf, avatarAi, avatarPick, setAvatarPick }) => {
  ...
  const runChatTurn = async (msgsForCtx, aiMsgId) => {
    ...
    const needsTool = /(代码|文件|目录|记忆|天气|健康|睡眠|步数|位置|城市|仓库|项目|看看|查一下|查查|读一下|改一下|找找|找到|去翻|读读|检查|确认.*(在|有)|还在吗|在哪|说句话|用语音|出声|朗读|念出来|掷骰|走棋|结算|大富翁|开局|记下来|写日记|记日记|留纸条|写觉察|记一笔|写碎片|留个)/.test(userText)
